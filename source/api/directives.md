...
或者，你可以直接将该指令绑定到一个对象。对象关键词 keys 将使特性列根据对应值予以设定。

本质上，普通特性中的 {{ Mustache }} 插值会被编译到计算后的 `v-attr` 指令当中。

从 0.12.8 开始，当 {{ Mustache }} 直接用在 `<input>` 控件的 `value` 特性时，其相应的 `value` 属性也会被设置。例如，`<input value="{% raw %}{{val}}{% endraw %}">` 不仅会更新特性值，也会设置其属性值。
